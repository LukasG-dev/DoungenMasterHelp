...
function loadNavbar() {
    fetch('/DoungenMasterHelp/navbar/navbar.html')
      .then(res => res.text())
      .then(data => {
  ...
  function saveDataAsJSON() {
    const data = {
      charaktere: extractCharacterData(document.getElementById('character-container')),
  ...
  function extractCharacterData(container) {
    ...
    entries.forEach(entry => {
      ...
      const [hpAktuell, hpMax] = (entry.textContent.match(/HP:\s*(\d+)\s*\/\s*(\d+)/) || [0, 0, 0]).slice(1).map(Number);
  ...
  function loadDataFromJSON(event) {
    ...
    reader.onload = function(e) {
      try {
        ...
        // Container leeren
        document.getElementById('character-container').querySelectorAll('.character.field').forEach(el => el.remove());
        document.getElementById('npc-container').querySelectorAll('.character.field').forEach(el => el.remove());
        document.getElementById('quest-container').querySelectorAll('.quest.field').forEach(el => el.remove());
  ...
  function loadCharaktere(liste) {
    ...
    liste.forEach(c => {
      ...
      container.insertBefore(newElement, container.querySelector('.add-entry-block'));
    });
  }
